...
#create_t ="""create table exp_list(val varchar2(200))"""
...
#cursor.executemany("insert into exp_list(val) values(:val_list)",table_list)
Data = np.array(list(cursor.fetchall()))
...
#cursor.execute("drop table exp_list")
if str(val) == "*":
 for x in Data:
  # Establish the database connection
  connection1 = cx_Oracle.connect(username,password, servername+"/"+dbname)
